...
| JG-025 | queued (triaged 2026-09-02; owner reference render registered at `references/media/handle-rear/`) | Handle-rear realism pass — red button/LCD cluster per owner reference (buttons `#c8e6ff` blue → red, aluminum surround → red bezel + glossy black), LCD data readout (CanvasTexture/decal), button symbols, reveal-beat framing vs reference, `LcdFillLight` retune; code-side materials only, no GLB re-export, unit-role path (dead `ROLE_OVERRIDES` finding) | [Queued](../../TODO.md#queued) | [plan](plans/JG-025-handle-rear-realism.md) | — |

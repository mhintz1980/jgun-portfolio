...
| JG-024 | implemented + machine-verified 2026-09-02 — **FINISH: stopgap replaced by owner source `JGUN-1.glb` (13 screws incl. the 4 CAD-true top-level `90910A815` gearbox bolts, az 0/90/180/−90 r 32.3 z −69.6, shanks radial); pipeline re-run → 10.18 MB / ≈562K tris; rig retarget `GB_FASTENER_RE` `/90910A815/i` + occurrence/HANDLE disambiguation + bolt-GROUP tagging; ladder @0.52 byte-identical, bolts rest/pop/ride exact vs spec, perf 16.8 ms quantum (evidence §8)** — **owner visual ruling pending (browser at `?chapter=2`)** | Fine-tessellation JGun re-export with all 13 fasteners — gltf-transform simplify+Draco from `JGUN-1.glb`, 9 handle screws (`fastener` unit) + 4 radial gearbox bolts (`gb-fastener-N` units with radial pop + clutch ride), `blackOxideSteel` default | [Queued](../../TODO.md#queued) | [plan](plans/JG-024-fine-jgun-fastener-reswap.md) | [evidence](evidence/JG-024-fine-jgun-fastener-reswap-verification.md) |

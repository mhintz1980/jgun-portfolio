...
| JG-025 | in-progress 2026-09-05 on `zcode/jg-025-handle-rear` (worktree `jgun-portfolio-jg025`, off `main` @ `4158a6c`): implemented + machine-verified (materials/decal telemetry, de-stack pixel-proven, ladder byte-identical, p95 16.8 quantum, tiers green, +5.35 kB); owner visual ruling pending at the `:4174` dwell | Handle-rear realism pass — red button/LCD cluster per owner reference (buttons `#c8e6ff` blue → red, aluminum surround → red bezel + glossy black), LCD data readout (CanvasTexture/decal), button symbols, reveal-beat framing vs reference, `LcdFillLight` retune; code-side materials only, no GLB re-export, unit-role path (dead `ROLE_OVERRIDES` finding); + fixed the pre-existing lcd-buttons merge-stack defect (per-part `lcd-button-N` units) | [Active](../../TODO.md#active) | [plan](plans/JG-025-handle-rear-realism.md) | [evidence](evidence/JG-025-handle-rear-verification.md) |

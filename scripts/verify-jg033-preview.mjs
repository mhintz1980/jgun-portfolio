...
const FULL_CUT = -.15 // evaluateShot(1).plane
...
try {
  for (const [name, width, height, quality] of [['desktop', 1440, 900, 'full'], ['portrait', 390, 844, 'full'], ['tablet', 768, 1024, 'full'], ['lite', 390, 844, 'lite']]) {
    ...
    const response = await page.goto(`${base}/?study=rl300&quality=${quality}`, { waitUntil: 'networkidle' })
    ...
    report.checks.ruledParts = assertRuling(await page.evaluate(() => window.__quietMachine.parts))
    const images = {}
    for (const [shot, u] of [['exterior', 0], ['section', .52], ['intake', 1], ['reverse-section', .52], ['reverse-exterior', 0]]) {
      await seek(page, u)
      ...
      if (name === 'desktop' && !shot.startsWith('reverse') && process.env.CAPTURE_POSTERS === '1') {
        fs.mkdirSync('public/images', { recursive: true })
        await page.locator('.qm-stage').screenshot({ path: `public/images/rl300-${shot}-preview.png` })
      }
    }
    const reverse = { section: compare(images.section, images['reverse-section']), exterior: compare(images.exterior, images['reverse-exterior']) }
    assert(reverse.section.fraction < .0001 && reverse.exterior.fraction < .0001, 'reverse must reproduce the rendered frame')
    await seek(page, .52)
  ...
  if (process.env.CAPTURE_POSTERS !== '1') {
    ...
    await p.close()
    for (const scenario of ['poster', 'asset-failure']) {
      const page = await browser.newPage()
      if (scenario === 'asset-failure') await page.route('**/models/msp-enclosure.glb', route => route.abort())
      await page.goto(`${base}/?study=rl300${scenario === 'poster' ? '&quality=poster' : ''}`)
...
console.log(JSON.stringify({ failure: report.failure, build: report.build, checks: report.checks, errors: report.errors }, null, 2))

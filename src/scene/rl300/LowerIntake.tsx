...
export function createLowerIntake() {
  ...
  // Boundary of V2RL300-SAF-1047-5 in web Y-up: x ±.378, z .449..1.052, y .152.
  add([.756, .018, .028], [0, .153, .462]); add([.756, .018, .028], [0, .153, 1.038])
  add([.028, .018, .576], [-.364, .153, .75]); add([.028, .018, .576], [.364, .153, .75])
  for (let i = 0; i < 9; i++) add([.70, .008, .042], [0, .153, .505 + i * .061], Math.PI / 7)
  ...
  const routeMaterial = new MeshStandardMaterial({ color: '#93e3fa', emissive: '#62cdeb', emissiveIntensity: .7, roughness: .6 })
  const routes = [-.12, .12].map(x => {

 ...
 *
 * JG-035 Track A (2026-09-25): the sheet is oversized (~3× viewport) on every viewport, so
 * the intro is a reading path everywhere (the 2026-09-05 narrow-only pan is superseded):
 *   open tight on the title block -> full-sheet establishing beat -> pan the view block ->
 *   settle square-on to the primary elevation before the pulse starts.
 */
...
/**
 * Station list for the reading pan. Everything is derived from the sheet zones and the
 * viewport aspect, so a different viewport re-fits rather than drifting off the sheet.
 *
 * JG-035 Track A: the sheet is oversized on EVERY viewport, so desktop now gets the same
 * authored reading path the narrow branch had — open tight on the title block (the sheet's
 * signature furniture), pull up to a full-sheet establishing beat, pan the view block, then
 * settle square-on to the primary elevation at the pulse.
 */
function readingStations(layout: DrawingLayout, aspect: number): Station[] {
  ...
  return [
    { t: 0, x: titleX, y: titleY, distance: distanceForWidth(SHEET_ZONES.titleBlock.w * 2.2, aspect, 0.02) },
    { t: 0.06, x: titleX, y: titleY, distance: distanceForWidth(SHEET_ZONES.titleBlock.w * 2.2, aspect, 0.02) },
    { t: 0.14, x: 0, y: 0, distance: distanceForWidth(layout.width, aspect, 0.06) },
    { t: 0.2, x: 0, y: 0, distance: distanceForWidth(layout.width, aspect, 0.06) },
    { t: INTRO_PHASES.pulseStart - 0.06, x: viewsX, y: viewsY, distance: distanceForWidth(SHEET_ZONES.views.w, aspect, 0.02) },
    {
...
export function introCameraPose(
  ...
  const stations = readingStations(layout, aspect)
  let from = stations[0]
  let to = stations[0]

...
- [ ] **JG-024 — Fine-tessellation JGun re-export with missing fasteners** · [plan](project/work/plans/JG-024-fine-jgun-fastener-reswap.md) · [evidence](project/work/evidence/JG-024-fine-jgun-fastener-reswap-verification.md)
  ...
  - **Gearbox bolts (owner spec 09-02, evidence §7):** 4 radial bolts (axes ⊥ drivetrain, 90° apart, concentric, through P000245 into clutch housing) — absent from all exports; stopgap quartet injected at best-guess positions + per-bolt radial-pop rig code is committed/live. **NEXT: rebuild from `C:\Projects\CAD\JGUN-1.glb`** (Fine+Draco, carries the 4 bolts as top-level `90910A815` nodes with CAD-true transforms) — retarget `GB_FASTENER_RE`, read true positions from the file, drop the injector stopgap, re-verify. Kickoff prompt delivered 2026-09-02.

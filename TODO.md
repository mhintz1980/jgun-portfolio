...
- [ ] **JG-032 — Station-2 thermal visualization, dark-blue recolor & gated JGUN polish** · [plan](project/work/plans/JG-032-station2-thermal-visualization.md) · [evidence](project/work/evidence/JG-032-station2-thermal-verification.md)
  ...
  - **Machine gates GREEN 2026-09-08** (`verify-jg032` ALL PASS): A/B census vs baseline `477c9c3` = 54 allow-listed recolors + 151 intended panel-flip flags + 0 unexpected; airway AABB ±0.01; 6 acoustic + 5 thermal shells; CH.04 telemetry byte-identical (1e-4 quantized, rotations/perf excluded); JGUN gates live-verified inert at p=0.85; 48/48 tests, typecheck/build/station2 green, 0 declines/0 console errors; perf p50 +3.9 ms vs baseline (translucent-panel restore) disclosed in evidence. **Awaiting owner visual ruling** — suggested stops in the evidence file.
...
- [x] **JG-029 — Ring switch knurl restoration (OD only)** · [plan](project/work/plans/JG-029-ring-switch-knurl.md) · [evidence](project/work/evidence/JG-029-ring-switch-knurl-verification.md)
  ...
  - **CLOSED VERIFIED 2026-09-06 — A/B census vs HEAD `53b6d24`:** ringSwitch bucket 88 → 4,292 v (exact GLB count) with 4,292/4,292 UVs and normalMap (182, 17); pins/plungers → clutchSteel 12,810 v; vertex totals conserved; handle multi-prim electronics completed their authored routing (same defect); nothing else changed; `verify-jg028` 5/5, `verify-jg027` pixel gates PASS, typecheck/tests/station2 green, 0 console errors. Owner visual ruling at `:4173` pending scroll review.

- [x] **JG-028 — Handle realism pass: black anodized body, trigger blade & stainless P001928** · [plan](project/work/plans/JG-028-handle-realism.md) · [evidence](project/work/evidence/JG-028-handle-realism-verification.md)
  - Bring the Handle Assembly and hardware to CAD realism matching Mark's Onshape references: retune `anodizedAluminum` from `#1a1a1e` to `MeshStandardMaterial` `#040404` (roughness 0.26, metalness 0.98, envMapIntensity 1.00, zero clearcoat) so the handle body and trigger blade are deep satin black seamlessly matching the gearbox cylinder; give reversing spool `P001928` a stainless steel finish (`#c2c6cb`, roughness 0.28, metalness 0.92); fix the latent generic mesh-name classification defect in `nodeRoles.ts`; and retain all fasteners in authentic Black Oxide finish (`#0d0d0d`, `blackOxideSteel`).
  - **Required proof:** `verify-jg028-handle-realism.mjs` (asserts handle body color `#040404`, `P001928` role `stainlessSteel` `#c2c6cb`, fasteners `blackOxideSteel` `#0d0d0d`, rigid handle explosion to `-0.354`, and CH.01 luminance parity), fresh `:4173` preview telemetry, same-frame before/after pairs, `npm run typecheck`, `npm test`, `scripts/check-station2-contract.mjs`, owner visual ruling at `:4173` CH.01 hero view.
  - Owner rulings 2026-09-06: handle + trigger black anodized aluminum; fasteners black oxide; P001928 stainless steel.
  - **CLOSED VERIFIED 2026-09-06 — owner visual ruling PASS ("looks good apprived. Update docs.") at `http://localhost:4173/?chapter=0`**. Machine gates: all 5 assertions PASS, 0 console errors, rigid explosion deltaZ = -0.3540 verified, typecheck/test/check:station2 green.
  - Independent regression sweep 2026-09-06 CLEAN: A/B live census vs pre-JG-028 HEAD `a76fc73` + telemetry at 8 stops + pixel diffs — nothing outside scope affected; the `nodeRoles` fix additionally routes ring-switch internals and handle electronics to their pre-existing authored `ROLE_OVERRIDES` (first build where they render; visible during rear extraction). [sweep evidence](project/work/evidence/JG-028-regression-sweep.md)

- [x] **JG-027 — LCD cluster restoration & illumination** · [plan](project/work/plans/JG-027-lcd-cluster-illumination.md) · [evidence](project/work/evidence/JG-027-lcd-cluster-verification.md)
  ...
  - **CLOSED VERIFIED 2026-09-06 — owner visual ruling PASS ("Looks good, pass it.") at `?dwell=lcd`** (round 2, after rev 1 rotated the bezel+readout 90° CW per the owner's portrait-window defect report; buttons ruled correct as-is). Machine record: all 5 surfaces facing 0.92–0.97, before-frame cluster pixels were literal zero → after 1,734+ white / 268+ red in per-decal rects, long-axis-vertical + reads-top-to-bottom gates green, both viewports, `?dwell=lcd` paced 0.47299, reduced-motion clean.
